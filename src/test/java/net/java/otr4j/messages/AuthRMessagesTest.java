...
public final class AuthRMessagesTest {
    ...
    public void testValidateSuccessful() throws OtrCryptoException, ValidationException {
        ...
        final ClientProfilePayload theirPayload = ClientProfilePayload.sign(new ClientProfile(SMALLEST_TAG,
                        theirLongTermKeyPair.getPublicKey(), forgingKey, singleton(OTRv.FOUR),
                        theirDSAKeyPair.getPublic()), Long.MAX_VALUE / 1000, theirDSAKeyPair,
                theirLongTermKeyPair);
        ...
        final ClientProfilePayload ourPayload = ClientProfilePayload.sign(new ClientProfile(HIGHEST_TAG,
                        ourLongTermKeyPair.getPublicKey(), ourForgingKey, singleton(OTRv.FOUR), ourDSAKeyPair.getPublic()),
                Long.MAX_VALUE/1000, ourDSAKeyPair, ourLongTermKeyPair);
        ...
        validate(message, ourPayload, "alice@network", "bob@network",
    ...
    public void testValidateInstanceTagMismatch() throws OtrCryptoException, ValidationException {
        ...
        final DSAKeyPair theirDSAKeyPair = DSAKeyPair.generateDSAKeyPair();
        final Point forgingKey = EdDSAKeyPair.generate(RANDOM).getPublicKey();
        final ClientProfilePayload theirPayload = ClientProfilePayload.sign(new ClientProfile(new InstanceTag(257),
                        theirLongTermKeyPair.getPublicKey(), forgingKey, singleton(OTRv.FOUR),
                        theirDSAKeyPair.getPublic()), Long.MAX_VALUE / 1000, theirDSAKeyPair,
                theirLongTermKeyPair);
        ...
        final AuthRMessage message = new AuthRMessage(OTRv.FOUR, SMALLEST_TAG, HIGHEST_TAG, theirPayload, theirX,
                theirA, sigma);
        validate(message, ourPayload, "alice@network", "bob@network", ourY, ourB, "?OTRv4?");
    }
